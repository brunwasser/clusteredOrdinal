...
set.seed(123)
dt <- data.table(mvrnorm(n=1000000,
              mu=mn,
              Sigma=sig))


repeat_rows <- function(dt) {
 dt[rep(1:.N, sample(2:8, .N, prob = c(.20, .40, .20, .10, .05, .03, .02), replace = TRUE))]
}
...
set.seed(101112)
u00 <- rnorm(cluster, mean=0, sd=.2)
set.seed(131415)
u01 <- rnorm(cluster, mean=0, sd=.05)
...
dtsub <- dt[sample(clusid, 10),]
boxplot(y ~ clusid, dtsub)

dt[sample(clusid, 10), boxplot(y ~ clusid)]
```
...
set.seed(init.seed)
u0j <- rnorm(nclus, 0, 0.8)


set.seed(init.seed+1)
u1jtemp <- rnorm(nclus, 0, 0.20)
u1j <- complement(u1jtemp, 0.99, u0j)

set.seed(init.seed+2)
u2jtemp <- rnorm(nclus, 0, 0.10)
...
estlist1$ci.type <- factor(rep(c('Regular','Robust'), each = nsim))
```
